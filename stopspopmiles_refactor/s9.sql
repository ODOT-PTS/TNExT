with census as (
  ...
  where 
    ...
    And block.urbanid = 'AREAID' 
    and block.regionid = 'GEOID' 
  group by 
    block.blockid
), 
employment as (
...
routes as (
  select 
    max(
      ST_Length(
        st_transform(
          st_intersection(regions.rshape, maps.shape), 
          2993
        )
      )/ 1609.34
    ) as length, 
  ...
  from 
    gtfs_trips trip 
    inner join census_urbans_trip_map maps on trip.id = maps.tripid cross 
    join regions 
  where 

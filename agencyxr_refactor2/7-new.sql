...
trips as (
  ...
  from 
    ...
    INNER JOIN census_urbans_trip_map map ON trip.id = map.tripid 
    AND trip.agencyid = map.agencyid 
    INNER JOIN census_tracts_trip_map maps on trip.id = maps.tripid 
    and trip.agencyid = maps.agencyid 
  where 
    trip.agencyid = 'TRIMET' 
    AND map.urbanid = '71317' 
    AND maps.tractid = '41051000100'
), 
service as (
  select 
    COALESCE(

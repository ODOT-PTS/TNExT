...
var congraphobj;
var edgeStyle = {}; // The object that contains the style setting of edges on
					// the graph.
edgeStyle['normal'] = {
	color : 'blue',
	opacity : 0.3,
							...
							.each(
									...
									function(index, item) {
										...
										if (vertices[item.a1ID] != undefined) {
											...
											vertex1
													.bindPopup(
															'<table style="font-size:100%"><tr><td>Agency&nbsp;ID:&nbsp;</td><td nowrap>'
																	+ item.a1ID
																	+ '</td></tr><tr><td>Agency&nbsp;Name:&nbsp;</td><td nowrap>'
																	+ item.a1name
																	+ '</td></tr></table>',
															{
																autoPan : false
													...
													.on(
															...
															function(e) {
																...
																vertex1
																		.setStyle({
																			weight : 6,
																			opacity : agencyStyle['hover'].opacity
																		});
															})
													.on(
															'mouseout',
															function(e) {
																...
																vertex1
																		.setStyle({
																			weight : 4,
																			opacity : agencyStyle[vertex1.status].opacity
																		});
															});
										...
										if (item.connections.size != 0) {
														...
														.on(
																...
																function(e) {
																	...
																	vertex2
																			.setStyle({
																				weight : 4,
																				opacity : agencyStyle[vertex2.status].opacity
																			});
																});
											...
											var edgeCoordinates = getCoordinatesForEdge(
													vertex1, vertex2);
											var edge = L
													.polyline(
															edgeCoordinates,
															{
																color : edgeStyle['normal'].color,
																opacity : edgeStyle['normal'].opacity
															});
											edge
													.on(
															'mouseover',
...
//																edge.closePopup();
											...
											if (vertexEdgesMap[vertex2.id] === undefined)
												vertexEdgesMap[vertex2.id] = [];
											vertexEdgesMap[vertex2.id]
													.push(vertex1);
											vertexEdgesMap[vertex2.id]
													.push(edge);
					...
					// /////// loading agency list and
					// check-boxes///////////////////
					// get agency list
					var agencies = {};
					var sortedAgencyList = []; // used to sort the list of
												// agencies in connectivity
												// graph dialog box
					var html = '';
					...
					html += '<th><img src="../resources/images/tooltip.png" style="cursor:help; width:100%;" title="The checkbox highlights the connectivity graph element that represents the agency" alt="1"></th>';
					html += '<th><img src="../resources/images/tooltip.png" style="cursor:help; width:100%;" title="The checkbox highlights the agency, as well as the agencies connected to it and the connections" alt="1"></th>';
					html += '<th><img src="../resources/images/tooltip.png" style="cursor:help; width:100%;" title="The checkbox highlights all parts of the transit graph where one can get to, starting from the associated agency" alt="1"></th>';
					...
					for (var i = 0; i < sortedAgencyList.length; i++) {
						...
						html += '<tr><th><input type="checkbox" class="conGraphCheckBox" id="'
								+ id
								+ '" data-_case="1" onchange="myFunction(this)"></th>'
								+ '<th><input type="checkbox" class="conGraphCheckBox" id="'
								+ id
								+ '" data-_case="2" onchange="myFunction(this)"></th>'
								+ '<th><input type="checkbox" class="conGraphCheckBox" id="'
								+ id
								+ '" data-_case="3" onchange="myFunction(this)"></th>'
...
/**
 * returns an array of coordinates that shows the shortest distance between to
 * shapes (agencies) on the map.
 * 
 */
function getCoordinatesForEdge(v1, v2) {
	...
	output[1] = {
		lat : (output[0].lat + output[2].lat) / 2,
		lng : (output[0].lng + output[2].lng) / 2
	}; // Setting the center point on the edge to locate the popup.
	return output;
}
...
/**
 * Toggles the opacity of the selected agencies along with all the edges and
 * agencies connected to it.
 * 
 * @param agencyEdgesMap
 *            (object)
 * @param vertex
 *            (object)
 * @param opacity
...
/**
 * Toggles the opacity of all the edges and vertices connected to the selected
 * vertex.
 * 
 * @param agencyEdgesMap
 *            (HashMap)
 ...
 *            (string)
 */
function toggleVertexConnections(agencyEdgesMap, edgeVerticesMap, v,
		agency_style, edge_style) {
	var stop = true;
	v.setStyle({
		fillOpacity : agencyStyle[agency_style].opacity,
		opacity : agencyStyle[agency_style].opacity,
...
/**
 * Makes the leaflet object of the agency "x". If the agency is centralized, a
 * marker is returned, esle a FeatureGroupe.
 * 
 * @param ConGraphAgency
 *            (object)
 * @returns {L.Path}
 */
function makeMarker(x) {
	if (x.centralized) {
		var marker = new L.CircleMarker(
				[ x.vertices[0].lat, x.vertices[0].lng ], {
					radius : 3,
		...
		for (var i = 0; i < x.vertices.length; i++) {
			var marker = new L.CircleMarker([ x.vertices[i].lat,
					x.vertices[i].lng ], {
				radius : 3,
				color : agencyStyle['normal'].color,
				weight : 2,
...
function myFunction(input) {
	...
	$.each($(":input#" + input.id + ".conGraphCheckBox"),
			function(index, item) {
				if (item.dataset._case != myCase) {
					item.checked = false;
					toggleVertexConnections(vertexEdgesMap, edgeVerticesMap,
							vertices[item.id], 'normal', 'normal');
				}
			});
	...
	if (myCase == 1) {
		...
		if (isChecked) {
			toggleVertexNeighbors(vertexEdgesMap, vertex, 'highlight',
					'highlight');
		} else {
		...
		if (isChecked) {
			toggleVertexConnections(vertexEdgesMap, edgeVerticesMap, vertex,
					'highlight', 'highlight');
		} else {
			toggleVertexConnections(vertexEdgesMap, edgeVerticesMap, vertex,
					'normal', 'normal');
